	...
	if ( $shipping_method_id && 'paccofacile_shipping_method' === $shipping_method_id ) {
		...
		if ( array_key_exists( 'data', $response_preventivo ) ) {
			...
			if ( array_key_exists( 'data', $response_ordine ) ) {
				...
				update_post_meta( $order_id, 'order_parcels', $parcels );
				// }
...
add_filter( 'woocommerce_order_formatted_billing_address' , 'pfwc_default_billing_address_fields', 10, 2 );
/**
 * Paccofacile billing building number to billing data
 *
 * @param [array] $fields fields array.
 * @param [obj] $order Order object.
 * @return void
 */
function pfwc_default_billing_address_fields( $fields, $order ) {
    $fields['billing_building_number'] = $order->get_meta('_billing_building_number');
    return $fields;
}

add_filter( 'woocommerce_order_formatted_shipping_address' , 'pfwc_default_shipping_address_fields', 10, 2 );
/**
 * Paccofacile shipping building number to billing data
 *
 * @param [array] $fields fields array.
 * @param [obj] $order Order object.
 * @return void
 */
function pfwc_default_shipping_address_fields( $fields, $order ) {
    $fields['shipping_building_number'] = $order->get_meta('_shipping_building_number');
    return $fields;
}


add_action( 'woocommerce_admin_order_data_after_shipping_address', 'pfwc_shipping_building_number_info' );
/**
 * Paccofacile building number in order admin view
 *
 * @param [type] $order Order object.
 * @return void
 */
function pfwc_shipping_building_number_info( $order ) {
	$shipping_building_number = $order->get_meta('_shipping_building_number');
	echo '<p><b>' . esc_html__( 'Building number', 'paccofacile-for-woocommerce' ) . '</b>: ' . esc_html( $shipping_building_number ) . '</p>';
}
...
function pfwc_credit_meta_box( $post_or_order_object ) {
	...
	$order_parcels = get_post_meta( $order->get_id(), 'order_parcels', true ) ? get_post_meta( $order->get_id(), 'order_parcels', true ) : '';

	if ( $order_parcels ) {
		$count_parcels = count( $order_parcels );
		for ( $i = 0; $i < $count_parcels; $i++ ) {
			$order_weight += $order_parcels[ $i ]['weight'];
		}
	}
	...
	<?php } else { ?>
		<div class="paccofacile_pay_order_form">
			...
			<?php /* Questo campo input nelle nuove versioni di WooCommerce crea problemi nel salvataggio perché va in conflitto con wc-orders
			<input type="hidden" name="post_type" value="shop_order">
			*/ ?>
...
function pfwc_tracking_meta_box( $post_or_order_object ) {
	...
	if ( ! empty( $order_tracking ) ) {
		...
		<p><?php _e('You have not yet received shipment tracking', 'paccofacile-for-woocommerce'); ?></p>
	<?php }
}
...
function pfwc_parcels_meta_box( $post_or_order_object ) {
	...
	$order_parcels = get_post_meta( $order->get_id(), 'order_parcels', true );
...
function pfwc_quote_and_save_by_woo_order( $order, $action = null ) {
	...
	$paccofacile_order_payload = get_post_meta( $order->get_id(), 'paccofacile_order_payload', true ) ? get_post_meta( $order->get_id(), 'paccofacile_order_payload', true ) : '';

	if( $paccofacile_order_payload ) {
		$service_id = $paccofacile_order_payload['shipment_service']['service_id'];
	} else {
		$service_id = false;
	}
	...
	$response_quote = $paccofacile_api->calculate_quote( $store_country, $store_state, $store_postcode, $store_city, $destination_country, $destination_province, $destination_postcode, $destination_city, $parcels, $service_id );
	...
	if ( array_key_exists( 'data', $response_quote ) && ! empty( $response_quote['data'] ) ) {
		...
		update_post_meta( $order->get_id(), 'order_parcels', $parcels );
		// }
